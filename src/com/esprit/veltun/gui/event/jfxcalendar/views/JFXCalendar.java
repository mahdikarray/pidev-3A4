...
public class JFXCalendar extends StackPane {
	...
	private void initToolBar() {
		...
		FontAwesomeIconView backIcon = new FontAwesomeIconView(
				FontAwesomeIcon.ARROW_LEFT);
		backIcon.setSize("24");
		// The Navicon Button on the top left
		backButton = new JFXButton();
		backButton.getStyleClass().add("circle_buttom");
		backButton.setGraphic(backIcon);

		backButton.setOnAction((ActionEvent evt) -> {
			backToSearch();
		});
		...
		try {
			weatherData = weatherService.getMeteoData();
			stringBuilder.append(" ( ").append(weatherData.getCurrent_weather().getTemperature()).append("°C").append(")");
		} catch (Exception e) {
		...
		// The Calendar Label
		Label calendarLabel = new Label("Calendar" + stringBuilder.toString());
		...
		// Add all components
		toolPane.getChildren().addAll(backButton,menuButton, calendarLabel, spacerPane, todayButton,
	...
	public void backToSearch() {
		...
		try {
			...
			thisStage.setTitle("Gestion des evenements");
